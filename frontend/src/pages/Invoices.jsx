...
import GenerateMediaButton from '../components/GenerateMediaButton.jsx';
...
export default function Invoices() {
  ...
  useEffect(() => {
    ...
    // Pobierz pozycje za media
    fetch(`${API_URL}/media/records`, {
      credentials: 'include',
    })
      .then(res => res.json())
      .then(data => {
        setMediaRecords(data);
        setMediaLoading(false);
      })
      .catch(err => {
        console.error('Błąd ładowania pozycji za media:', err);
        setMediaLoading(false);
      });
  }, []);
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center">
              <div className="h-12 w-12 bg-emerald-100 rounded-lg flex items-center justify-center">
                <svg className="h-6 w-6 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" width="24" height="24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Pozycje za media</p>
                <p className="text-2xl font-bold text-gray-900">{mediaRecords.length}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-50 to-teal-50 px-6 py-4 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-600">
                  {mediaRecords.length} pozycji za media
                </p>
              </div>
                        ...
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              {record.status === 'pending' ? 'Oczekuje' :
                               record.status === 'calculated' ? 'Obliczone' : 'Finalizowane'}
                            </span>
                          </div>
                          <p className="text-sm text-gray-600 mt-1">
                            {record.Client?.name || 'Nieznany klient'} • {record.year}-{String(record.month).padStart(2, '0')}
                          </p>
                          {record.MediaTemplate && (
                            <p className="text-sm text-gray-500 mt-1">
                              Wzór: {record.MediaTemplate.formula}
                            </p>
                          )}
                        </div>
                      </div>
